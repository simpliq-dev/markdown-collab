...
2) GPT produces formal docs (**PRFAQ.md**, **PRD.md**, **PLAN.md**) that become the working contract.  
...
5) If scope changes, update docs/PLAN first, then resume coding.
...
- **No broken builds**: do not leave the repo in a failing state unless explicitly instructed and clearly recorded in PLAN.md.
- **Small, coherent chunks**: finish a slice end-to-end (incl. validation) before starting the next.
- **Update PLAN.md every time** you complete a chunk, make a decision, or hit a blocker. PLAN.md is the memory.
...
- **No silent scope creep**: implement what is in PLAN.md. If you discover missing requirements, write them as Open Questions in PLAN.md and stop.
...
- `PLAN.md` - **the execution contract + TODO list**. Contains milestones, tasks, acceptance checks, worklog, decisions, and blockers.
...
3. `PLAN.md` (active execution contract + task list).
4. `docs/PRD.md`
5. `docs/PRFAQ.md`
...
- **Do not guess.**
- Record it in `PLAN.md` under **Open Questions** (and optionally propose a recommended resolution).
...
- `docs/WORKLOG.md` - short, append-only log of work done (useful if PLAN.md gets long).
...
3) Consistency check (do not "fix" silently):
   - Verify `PLAN.md` includes all mandatory sections required in **3.1**.
...
4) Transcript reconciliation (one-time, if `docs/IDEATION-TRANSCRIPT.md` exists and is real content):
   ...
   - Compare transcript vs `docs/PRFAQ.md`, `docs/PRD.md`, `PLAN.md`.
   - If the transcript contains details needed to implement correctly (examples, tech stack, UI decisions, constraints) that are missing/contradictory in the formal docs:
     ...
     - **ask permission before editing** `docs/PRFAQ.md`, `docs/PRD.md`, `PLAN.md`.
   - After reconciliation is complete: record a `PLAN.md` **Decision Log** entry (append-only) noting that the transcript was reconciled (one-time) and the date.
...
5) Create a minimal directory skeleton *only if absent*:
   - `docs/` (if missing)
   - Any directories explicitly referenced by `PLAN.md` tasks (create them, empty).
6) Scan the repo to find how to **build**, **test**, **lint**, and **run** the project:
   ...
   - If you cannot determine the commands, add an **Open Question** in PLAN.md:
...
Do not add language-specific tooling, package managers, or scaffolds unless PLAN.md explicitly calls for it.

---

## 3) PLAN.md format contract (TODO list + memory)

`PLAN.md` must remain easy to scan and must contain checkboxes the agent can update.
...
1) **Select the next chunk**
   ...
   - If tasks are too large/vague, split them in PLAN.md first (keep the intent intact).
...
3) Ask for a decision *only if necessary*; otherwise proceed with the safest reasonable option and record the decision in PLAN.md.
...
If any answer is “no,” fix it or record it as an explicit known gap in PLAN.md.
...
- If implementation reveals a new requirement, constraint, or ambiguity:
  - update **PLAN.md first** (Open Question or Decision),
